                            ...
                            {conversations.map(x =>
                                <div className="chat-connections" key={x.chats._id}>
                                    <Link onClick={() => setIsSelected(true)} to={`/messages/${x.chats._id}`}>
                                        {x.isBuyer ?
                                            <><img src={x.chats.seller && x.chats.seller.avatar} alt="user-avatar" /> <span>{x.chats.seller && x.chats.seller.name}</span></>
                                            :
                                            <><img src={x.chats.buyer && x.chats.buyer.avatar} alt="user-avatar" /> <span>{x.chats.buyer && x.chats.buyer.name}</span></>
                                        }
                                    </Link>
                                </div>)
                            }
                    ...
                    {isSelected &&
                        <>
                            <div className="chat-selected-header col-lg-12">
                                {selected.isBuyer ?
                                    <Link to={`/profile/${selected.chats.seller && selected.chats.seller._id}`}>
                                        <img src={selected.chats.seller && selected.chats.seller.avatar} alt="user-avatar" />
                                        <span>{selected.chats.seller && selected.chats.seller.name}</span>
                                    </Link>
                                    :
                                    <Link to={`/profile/${selected.chats.buyer && selected.chats.buyer._id}`}>
                                        <img src={selected.chats.buyer && selected.chats.buyer.avatar} alt="user-avatar" />
                                        <span>{selected.chats.buyer && selected.chats.buyer.name}</span>
                                    </Link>
                                }
                            </div>
                            {alertShow &&
